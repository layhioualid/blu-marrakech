...
export default function ProgramHours() {
  ...
  return <section id="programme" className="section-pad bg-[#05070c]">
    <div className="site-container">
      ...
      <div className="mt-12 grid gap-2 md:grid-cols-2 lg:grid-cols-7">{weeklyProgram.map((event, i) => <MotionReveal key={event.day} delay={i * .04}><article className={`group relative flex min-h-64 flex-col overflow-hidden border p-5 transition duration-500 hover:-translate-y-1 ${event.featured ? "border-gold/60 shadow-gold" : i === today ? "border-electric shadow-glow" : "border-white/10 hover:border-electric/50"}`}><Image src={event.image} alt={event.title} fill className="object-cover opacity-45 transition duration-700 group-hover:scale-110 group-hover:opacity-65" sizes="(max-width: 768px) 100vw, (max-width: 1024px) 50vw, 15vw"/><div className="absolute inset-0 bg-gradient-to-t from-ink via-ink/65 to-ink/15"/><div className="relative flex items-start justify-between"><span className="font-display text-3xl text-white/50">{event.short}</span>{event.featured && <span className="text-gold">★</span>}</div><div className="relative mt-auto"><p className="text-[8px] uppercase tracking-[.2em] text-gold">{event.day}</p><h3 className="mt-2 font-display text-xl leading-tight">{event.title}</h3><p className="mt-3 flex items-center gap-2 text-[9px] text-white/55"><Clock3 size={11}/>{event.time}</p></div></article></MotionReveal>)}</div>
